...
libri_dir=data/local/dict_libri

if [ $dict_type = "libri" ]; then
  if [ $stage -eq -1 ]; then
  ...
  if [ $stage -le 1 ]; then
    ...
    cp $libri_dir/{nonsilence_phones,silence_phones,optional_silence,extra_questions,lexicon}.txt \
      $dir
  fi
fi

if [ $dict_type = "wsj" ]; then
  ...
  if [ $stage -le 1 ]; then
    ...
    # copy silence, nonsilence and optional silence phones from wsj dict
    cp ${wsj_dir}/{nonsilence_phones,silence_phones,optional_silence,extra_questions,lexicon}.txt \
      $dir
    cp ${wsj_dir}/cleaned.gz $dir
  fi
