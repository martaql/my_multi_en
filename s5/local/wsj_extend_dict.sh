...
if [ $# -ne 2 ]; then
  echo "Usage: local/wsj_train_lms.sh old/dict/dir /foo/bar/WSJ/13-32.1/"
  exit 1
fi
if [ "`basename $2`" != 13-32.1 ]; then
...
srcdir=$2
#mkdir -p $dir
cp -r $old_dir/* $dir # Various files describing phones etc.
                      # are there; we just want to copy them
                      # as the phoneset is the same.
rm $dir/lexicon.txt   # we don't want this.
rm $dir/lexiconp.txt  # we don't want this either.
mincount=2 # Minimum count of an OOV we will try to generate a pron for.

[ ! -f $old_dir/cmudict/cmudict.0.7a ] && \
  echo "CMU dict not in expected place" && exit 1;

# Remove comments from cmudict; print first field;
# remove words like FOO(1) which are alternate prons:
# our dict format won't include these markers.
grep -v ';;;' $old_dir/cmudict/cmudict.0.7a |
